...
for t in "${TRANG[@]}"; do
  code=$(curl -sS -o /dev/null -w "%{http_code}" --max-time 20 "$BASE$t" 2>/dev/null)
  [ "$code" = "200" ] && pass "$t" || fail "$t trả về $code (mong đợi 200)"
done
...
ASSET=$(curl -sS --max-time 20 "$BASE/" 2>/dev/null | grep -o '/_astro/[A-Za-z0-9._-]*\.css' | head -1)
if [ -n "$ASSET" ]; then
...
for t in "/" "/banh-trung-thu-sheraton-ha-noi"; do
  HTML=$(curl -sS --max-time 20 "$BASE$t" 2>/dev/null)
  CANON=$(echo "$HTML" | grep -o '<link rel="canonical" href="[^"]*"' | sed 's/.*href="//;s/"//')
  MONG="$BASE$t"; MONG="${MONG%/}"; [ "$t" = "/" ] && MONG="$BASE"
  [ "$CANON" = "$MONG" ] && pass "canonical $t" || fail "canonical $t = '$CANON' (mong đợi '$MONG')"
...
curl -sS --max-time 20 "$BASE/hop-vip" 2>/dev/null | grep -qi 'name="robots"[^>]*noindex' \
  && pass "/hop-vip có noindex" || fail "/hop-vip THIẾU noindex (quy tắc C0)"

curl -sS --max-time 20 "$BASE/sitemap-index.xml" 2>/dev/null | grep -q "hop-vip" \
...
HOME_HTML=$(curl -sS --max-time 20 "$BASE/" 2>/dev/null)
...
for t in "/" "/banh-trung-thu-sheraton-ha-noi" "/banh-trung-thu-melia-ha-noi"; do
  H=$(curl -sS --max-time 20 "$BASE$t" 2>/dev/null)
  dinh=""
